...
export default function CustomEmailModal({ isOpen, onClose }: CustomEmailModalProps) {
  ...
  const handleSend = async () => {
    ...
    if (!recipientEmail.trim()) {
      setError("Email-adress måste fyllas i");
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Skicka Email" maxWidth="2xl">
      <div className="space-y-4 px-4 py-2">
        {/* Mottagare */}
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-2">Mottagare (Email)</label>
          <input
            type="email"
        ...
        <div className="flex gap-3 justify-end pt-4">
          ...
          <Knapp
            ...
            onClick={handleSend}
            disabled={
              sending || success || !recipientEmail.trim() || !subject.trim() || !message.trim()
            }
          />
        </div>
      </div>
