...
export default function Foretagsprofil({ company }: ForetagsprofilProps) {
  ...
  const onFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    await handlers.onUploadLogo(file);
          ...
          {state.isEditingCompany ? (
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <span className="text-white ml-2">
                  {state.foretagsProfil.adress || "Ej angett"}
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <span className="text-white ml-2">
                    {state.foretagsProfil.stad || "Ej angett"}
                  </span>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className="text-white ml-2">
                    {state.foretagsProfil.epost || "Ej angett"}
                  </span>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-3">
            ...
            />
            <Knapp
              text={state.isUploadingLogo ? "Laddar upp" : "Ladda upp logga"}
              onClick={triggerFileDialog}
              loading={state.isUploadingLogo}
              loadingText="Laddar..."
              disabled={state.isUploadingLogo}
            />
            {state.foretagsProfil.logo && (
              <Knapp
                text="🗑️ Ta bort"
                onClick={handlers.onRemoveLogo}
                disabled={state.isUploadingLogo}
                loading={state.isUploadingLogo}
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4 flex items-center justify-center min-h-[160px]">
          {state.foretagsProfil.logo ? (
            <div className="relative w-full max-w-xs">
              <Image
                src={getProxyImageUrl(state.foretagsProfil.logo)}
                alt="Företagslogga"
                width={state.foretagsProfil.logoWidth || 200}
                height={120}
            ...
            <span className="text-gray-500">Ingen logotyp uppladdad</span>
          )}
        </div>
      </div>
    </div>
