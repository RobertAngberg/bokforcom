...
import { genereraBokföringsrader } from "../../utils/bokforingsLogik";
import { valideraBokföring, formateraBeloppKronor } from "../../utils/bokforingsUtils";
...
export default function BokförKnappOchModal({
  ...
  if (freshLoading) {
    return (
      <Modal isOpen={true} onClose={onClose || (() => {})} title="Bokföring">
        <div className="p-8 text-center text-white">Hämtar färsk löneinformation...</div>
      </Modal>
    );
  }
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose || (() => {})}
      title={`📊 Bokföring${utbetalningsdatum ? ` - ${utbetalningsdatum.toLocaleDateString("sv-SE")}` : ""}`}
      maxWidth="4xl"
    >
      {/* INNEHÅLL */}
