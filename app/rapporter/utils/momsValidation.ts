import type { VerifikatValidation, TransaktionsPost, Transaction } from "../types/types";

/**
 * Validerar en enskild transaktion och dess poster
 * Returnerar varningar om något verkar fel
 *
 * INGEN databasfetching - bara validering av data som redan hämtats
 */
export function validateTransaction(
  transaction: Transaction,
  poster: TransaktionsPost[]
): VerifikatValidation {
  ...
  if (!poster || poster.length === 0) {
    ...
    const totalDebet = poster.reduce((sum: number, p: TransaktionsPost) => sum + (p.debet || 0), 0);
...
/**
 * Beräknar datumintervall för en period
 */
