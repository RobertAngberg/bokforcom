...
import { skapaKonton, importeraSieData, rensaDubblettkonton } from "./actions";
  ...
  return (
    <div className="text-center">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {/* DIMENSION DEBUG SEKTION */}
        {resultat?.debugLogs && (
          <div className="bg-slate-800 rounded-lg p-4 mb-6 text-left">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-white">🔍 Dimension Debug Information</h3>
              <button
                onClick={() => setShowDebugLogs(!showDebugLogs)}
                className="bg-cyan-600 hover:bg-cyan-700 text-white px-3 py-1 rounded text-sm"
              >
                {showDebugLogs ? "Dölj" : "Visa"} Debug Logs ({resultat.debugLogs.length})
              </button>
            </div>
              ...
              <div className="space-y-2">
                <div className="bg-slate-900 rounded p-3 max-h-96 overflow-y-auto">
                  <div className="text-xs font-mono text-green-400 space-y-1">
                    {resultat.debugLogs.map((log: string, index: number) => (
                      <div
                        key={index}
                        className={`
                        ${log.includes("🎯 DIMENSION HITTAD") ? "text-yellow-400 font-bold" : ""}
                ...
                <div className="mt-3 p-3 bg-yellow-500/20 border border-yellow-500 rounded">
                  <div className="text-yellow-400 text-sm">
                    <strong>💡 Copy-Paste Ready:</strong> Markera allt ovan och kopiera för att dela
                    med support/utvecklare
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="space-y-2 text-sm text-gray-400 mb-6">
          ...
          {resultat?.debugLogs && <div>• Dimension-analys tillgänglig</div>}
        </div>
